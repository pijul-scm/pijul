...
fn unsafe_apply<T>(db_internal:&Db<T>, db_external:&Db<T>, branch:&mut Branch<T>, db_contents:&mut Db<T>, changes:&[Change],
            ...
            Change::Edges{ref flag, ref edges} => {
                ...
                if *flag &DELETED_EDGE != 0 {
                    ...
                    while i<children.len() {
                        let mut j=0;
                        while j<parents.len() {
                            if ! connected(branch, &parents[j+1 .. j+1+KEY_SIZE],
...
pub fn apply<'b,T>(repository:&mut Transaction<T>, branch_name:&str, patch:&Patch, internal: &'b InternalKey, new_patches:&HashSet<&[u8]>)->Result<(),Error> {
    ...
    //let cursor_= unsafe {&mut *self.txn.unsafe_cursor(self.dbi_nodes).unwrap() };
    {
        ...
        let mut repair_missing_context= |db_nodes:&mut Branch<T>, direction_up:bool,c:&[u8] | -> Result<(),Error> {
                ...
                Change::Edges{ref flag,ref edges}=>{
                    ...
                    if (*flag)&DELETED_EDGE == 0 {
                        ...
                        for e in edges {
                            ...
                            try!(repair_missing_context(&mut db_nodes, (*flag)&PARENT_EDGE != 0,&u[..]));
                            try!(repair_missing_context(&mut db_nodes, (*flag)&PARENT_EDGE == 0,&v[..]));
                        }
                    } else // DELETED_EDGE
...
fn find_alive_relatives<T>(repository:&Transaction<T>, branch:&Branch<T>, a:&[u8], direction:u8, patch_id:&InternalKey, new_patches:&HashSet<&[u8]>,
...
fn reconnect_zombie_folder<T>(branch:&mut Branch<T>, a:&[u8], patch_id: &InternalKey)->Result<(),Error> {
    fn connect<T>(branch:&mut Branch<T>,
               a:&[u8],
               patch_id:&InternalKey,
               edges:&mut Vec<u8>) {
